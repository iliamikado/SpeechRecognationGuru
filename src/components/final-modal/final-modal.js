...
import { putResource, postResource } from '../../service/service';
...
class FinalModal extends Component {
    ...
    render() {
        ...
        const index = scores.findIndex(({name}) => (name === this.props.username));
        ...
        if (index === -1) {
            ...
            scores[index].flag = true;
            scores[index].score = this.props.right - this.props.wrong;
        }

        scores.sort((a, b) => (b.score - a.score));

        const rows = scores.map(({name, score, flag}, i) => {
            return (
